...
DB_NAME="${DB_NAME:-kindleaf_real_deepseek_smoke_$(date +%s)}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/.tmp/smoke-real-deepseek-text}"
AUTH_HEADER="Authorization: Bearer ${API_TOKEN:-dev-token}"

server_pid=""

"$ROOT_DIR/scripts/check-real-provider-readiness.sh" --deepseek
...
wait_for_job() {
  ...
  echo "real DeepSeek text job did not finish: $job_id" >&2
...
echo "== Kindleaf real DeepSeek text smoke =="
...
echo "logs=$LOG_DIR"
echo "This script calls the real DeepSeek provider and may consume quota."
...
echo "2. start backend with real DeepSeek text provider"
(
  cd "$ROOT_DIR/server"
...
echo "3. verify real DeepSeek provider readiness"
...
if(p.data.provider !== 'deepseek') process.exit(1);
if(p.data.mode !== 'text' || p.data.real_text_ready !== true) process.exit(1);
...
const configuredEndpoint = process.env.DEEPSEEK_ENDPOINT_PATH || '/chat/completions';
if(!text.model || !text.endpoint) process.exit(1);
if(configuredEndpoint.startsWith('http://') || configuredEndpoint.startsWith('https://')) {
  if(text.endpoint !== configuredEndpoint) process.exit(1);
} else if(!text.endpoint.endsWith(configuredEndpoint)) {
  process.exit(1);
}
console.log('real_deepseek_provider=' + text.provider + '/' + text.model);
"
...
storybook_json=$(api POST "/api/workspaces/$workspace_id/storybooks" '{"title":"Real DeepSeek Smoke 普通绘本","age_group":"4-5 岁","use_scene":"班级共读","teaching_goal":"学习轮流等待和表达想法"}')
storybook_id=$(echo "$storybook_json" | json_get "if(p.data.type!=='plain') process.exit(1); console.log(p.data.id);")
child_json=$(api POST "/api/workspaces/$workspace_id/children" '{"nickname":"DeepSeek Smoke 儿童","age_group":"4-5 岁","classroom":"小一班","interests":["积木","贴纸"],"traits":["愿意尝试"],"focus":"等待轮流时容易着急"}')
child_id=$(echo "$child_json" | json_get "if(!p.data.id) process.exit(1); console.log(p.data.id);")

plan_json=$(create_and_wait_job "$workspace_id" "storybook_plan" '{"job_type":"storybook_plan","input_json":{"theme":"轮流等待","age_group":"4-5 岁","teaching_goal":"学习轮流等待和表达想法","use_scene":"班级共读"}}')
...
echo "real_deepseek_plan_job=$plan_job_id"

roles_json=$(create_and_wait_job "$workspace_id" "storybook_roles" "{\"job_type\":\"storybook_roles\",\"storybook_id\":\"$storybook_id\",\"input_json\":{\"title\":\"Real DeepSeek Smoke 角色\",\"theme\":\"轮流等待\",\"teacher_name\":\"林老师\"}}")
...
echo "real_deepseek_roles_job=$roles_job_id"

pages_json=$(create_and_wait_job "$workspace_id" "storybook_pages" "{\"job_type\":\"storybook_pages\",\"storybook_id\":\"$storybook_id\",\"input_json\":{\"page_count\":4,\"theme\":\"轮流等待\",\"teaching_goal\":\"学习轮流等待和表达想法\"}}")
...
echo "real_deepseek_pages_job=$pages_job_id"

storybook_after_generation=$(api GET "/api/workspaces/$workspace_id/storybooks/$storybook_id")
echo "$storybook_after_generation" | json_get "if(!p.data.roles.length) process.exit(1); if(!p.data.pages.length) process.exit(1); console.log('real_deepseek_text_applied=' + p.data.id);"

customization_json=$(create_and_wait_job "$workspace_id" "customization_plan" "{\"job_type\":\"customization_plan\",\"storybook_id\":\"$storybook_id\",\"input_json\":{\"child_id\":\"$child_id\",\"child_nickname\":\"DeepSeek Smoke 儿童\",\"intensity\":\"standard\",\"source_title\":\"Real DeepSeek Smoke 普通绘本\",\"interests\":[\"积木\",\"贴纸\"],\"traits\":[\"愿意尝试\"],\"focus\":\"等待轮流时容易着急\",\"parent_email\":\"parent@example.com\",\"guardian_phone\":\"138 0013 8000\",\"family_note\":\"爸爸近期出差\"}}")
customization_job_id=$(echo "$customization_json" | json_get "
if(p.data.output_json?.provider!=='deepseek') process.exit(1);
if(p.data.output_json?.mode!=='customization_plan') process.exit(1);
if(!p.data.output_json?.customization?.strategy) process.exit(1);
const audit = p.data.output_json?.privacy_audit;
if(audit?.redacted !== true) process.exit(1);
const labels = audit.labels || [];
if(!labels.includes('sensitive_field')) process.exit(1);
const output = JSON.stringify(p.data.output_json);
if(output.includes('DeepSeek Smoke 儿童') || output.includes('parent@example.com') || output.includes('138 0013 8000') || output.includes('爸爸近期出差')) process.exit(1);
console.log(p.data.id);
")
echo "real_deepseek_customization_job=$customization_job_id"
echo "real_deepseek_customization_privacy_redacted=ok"

echo "5. verify operator generation cost ledger"
AUTH_HEADER="Authorization: Bearer $operator_token"
...
const expected = ['$plan_job_id', '$roles_job_id', '$pages_job_id', '$customization_job_id'];
...
if(p.data.items.some((row)=>row.provider !== 'deepseek' || row.status !== 'succeeded')) process.exit(1);
console.log('real_deepseek_costs=' + p.data.items.length + '/' + p.meta.total);
"
AUTH_HEADER="$teacher_auth_header"

echo "== real DeepSeek text smoke ok =="
