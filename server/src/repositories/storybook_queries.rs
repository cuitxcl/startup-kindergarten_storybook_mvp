...
async fn storybooks_from_rows(
    ...
    for row in rows {
        ...
        let mut book = Storybook {
            ...
            story_style_id: row.try_get("", "story_style_id")?,
            visual_style_id: row.try_get("", "visual_style_id")?,
            visual_style_version: row.try_get("", "visual_style_version")?,
